...
namespace KinoConsole
{
  public class HelpPage : PhoneApplicationPage
  {
    ...
    private void webBrowser_NavigationFailed(object sender, NavigationFailedEventArgs e) => this.webBrowser.Navigate(new Uri("index.html", UriKind.Relative));

    private void WebBrowser_Loaded(object sender, RoutedEventArgs e) => this.webBrowser.Navigate(new Uri("http://kinoconsole.kinoni.com/wpsetup.html", UriKind.Absolute));
    ...
    private void SaveFilesToIsoStore()
    {
      ...
      {
        StreamResourceInfo resourceStream = Application.GetResourceStream(new Uri("Assets/Help/" + fileName, UriKind.Relative));
    ...
    private void SaveToIsoStore(string fileName, byte[] data)
    {
      ...
      using (BinaryWriter binaryWriter = new BinaryWriter((Stream) storeForApplication.CreateFile(fileName)))
      {
