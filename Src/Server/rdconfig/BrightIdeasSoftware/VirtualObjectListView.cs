  ...
  public class VirtualObjectListView : ObjectListView
  {
    ...
    public override IList CheckedObjects
    {
      get
      {
        ...
        {
          if (checkState.Value == CheckState.Checked && this.VirtualListDataSource.GetObjectIndex(checkState.Key) >= 0)
            checkedObjects.Add(checkState.Key);
        }
    ...
    protected new virtual int VirtualListSize
    {
      ...
      set
      {
        if (value == this.VirtualListSize || value < 0)
          return;
        if ((object) VirtualObjectListView.virtualListSizeFieldInfo == null)
    ...
    {
      return this.VirtualListDataSource != null && index >= 0 && index < this.GetItemCount() ? this.VirtualListDataSource.GetNthObject(index) : (object) null;
    }

    public override int IndexOf(object modelObject)
    {
      return this.VirtualListDataSource == null || modelObject == null ? -1 : this.VirtualListDataSource.GetObjectIndex(modelObject);
    }
    ...
    public override OLVListItem ModelToItem(object modelObject)
    {
      ...
      return objectIndex < 0 ? (OLVListItem) null : this.GetItem(objectIndex);
    }
    ...
    public virtual void ClearCachedInfo() => this.lastRetrieveVirtualItemIndex = -1;
    ...
    protected override CheckState? GetCheckState(object modelObject)
    {
      ...
      if (modelObject != null)
        this.CheckStateMap.TryGetValue(modelObject, out checkState);
    ...
    {
      return this.GroupingStrategy == null ? (IList<OLVGroup>) new List<OLVGroup>() : this.GroupingStrategy.GetGroups(parms);
    }
